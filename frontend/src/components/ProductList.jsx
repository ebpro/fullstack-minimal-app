...
export default function ProductList({ categories = [], backend, backendAvailable = null }) {
  ...
  const fetchPage = async (p = page, c = categoryFilter) => {
    setLoading(true)
    try {
  ...
  useEffect(() => { fetchPage(page, categoryFilter) }, [page])
    ...
    <div>
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <label htmlFor="category-filter" className="text-sm text-gray-600">Filter:</label>
          {/* Controlled select: value={categoryFilter} and onChange updates state */}
          <select id="category-filter" value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="border px-3 py-2 rounded">
        ...
        <div>
          <Link to="/add" className="px-4 py-2 bg-emerald-600 text-white rounded-md shadow">Add product</Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map(p => (
          <div key={p.id} className="bg-white rounded-lg shadow hover:shadow-lg overflow-hidden transition-shadow">
            {/* Entire card is clickable (Link wraps image and text) */}
            <Link to={`/product/${p.id}`} className="block">
              {/* Product image with fallback placeholder */}
              <div className="h-48 w-full overflow-hidden">
                <img src={p.image_url || 'https://placehold.co/600x400?text=No+Image'} alt="" className="w-full h-full object-cover" />
              </div>
              <div className="p-4">
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <div className="text-xl font-bold text-emerald-600">${Number(p.price).toFixed(2)}</div>
                  <div className="flex items-center gap-2">
                    {/* Edit and Delete buttons. Teaching note: onClick on the button
                        prevents the Link's navigation (event propagation stops). */}
                    <Link to={`/product/${p.id}/edit`} className="px-3 py-1 bg-blue-600 text-white rounded">Edit</Link>
                    <button className="px-3 py-1 bg-red-500 text-white rounded" onClick={() => handleDelete(p.id)}>Delete</button>
                  </div>
                </div>
              </div>
            </Link>
      ...
      <div className="flex items-center justify-center gap-4 mt-8">
        <button className="px-3 py-1 border rounded disabled:opacity-50" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={meta.page === 1}>Prev</button>
        <div className="text-sm text-gray-600">Page {meta.page} / {meta.total_pages} — {meta.total} items</div>
        <button className="px-3 py-1 border rounded disabled:opacity-50" onClick={() => setPage(p => Math.min(meta.total_pages, p + 1))} disabled={meta.page === meta.total_pages}>Next</button>
      </div>
